...
async_task<mac_ue_reconfiguration_response> du_ue_ric_configuration_procedure::handle_mac_config()
{
  ...
  // [PATCH: UE-to-Slice migration] Rebuild full scheduler UE config with LC list, then override the
  // rrm_policy.s_nssai of all DRB logical channels to the S-NSSAI provided by the RIC. This causes
  // inter_slice_scheduler::reconf_ue() to re-route the UE to the slice instance matching the new
  // S-NSSAI, achieving runtime UE-to-Slice rebinding without RRC reconfiguration / PDU re-anchor.
  mac_request.sched_cfg = create_scheduler_ue_config_request(*ue, *ue->resources);

  const auto& policy = request.rrm_policy_ratio_list[0];
  if (!policy.policy_members_list.empty() && mac_request.sched_cfg.lc_config_list.has_value()) {
    const s_nssai_t new_nssai = policy.policy_members_list[0].s_nssai;
    unsigned drb_count = 0;
    for (auto& lc : *mac_request.sched_cfg.lc_config_list) {
      if (lc.lcid >= LCID_MIN_DRB) {
        lc.rrm_policy.s_nssai = new_nssai;
        ++drb_count;
      }
    }
    srslog::fetch_basic_logger("E2SM-RC").info(
        "[PATCH] UE-to-Slice migration: f1ap_id={} -> {} (overrode {} DRB LCs)",
        request.ue_id, new_nssai, drb_count);
  }

  // Configure UE resource allocation parameters.
  if (!mac_request.sched_cfg.res_alloc_cfg.has_value()) {
  ...
  res_alloc_cfg.rrm_policy_group = policy;
